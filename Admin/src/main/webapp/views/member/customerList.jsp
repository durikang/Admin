...
<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"
	import="java.util.ArrayList,java.util.List ,com.global.customer.model.*"%>
...
<link href="${contextPath}/resources/master.css" rel="stylesheet"
	type="text/css" />
<link href="${contextPath}/resources/member/css/memberListPwd.css"
	rel="stylesheet" type="text/css" />
<link href="${contextPath}/resources/member/css/memberList.css"
	rel="stylesheet" type="text/css" />
	...
	<div align="center">
		...
		<div class="form-container" id="filterForm">
			<form method="get" action="${contextPath}/customerList.go">
				<label> <input type="checkbox" name="status" value="N"
					<c:if test="${param.status == 'N'}">checked</c:if>> 회원
				</label> <label> <input type="checkbox" name="status" value="Y"
					<c:if test="${param.status == 'Y'}">checked</c:if>> 탈퇴 회원
				</label> <label> 정렬: <select name="sort">
						<option value="asc"
							<c:if test="${param.sort == 'asc'}">selected</c:if>>오름차순</option>
						<option value="desc"
							<c:if test="${param.sort == 'desc'}">selected</c:if>>내림차순</option>
				</select>
		...
		<table border="1">
			<tr>
				<td class="table_header button" colspan="9" align="right">전체 회원
					수 : ${count}</td>
			</tr>
			<tr>
				<th>아이디</th>
				...
				<c:when test="${not empty list}">
					<c:forEach items="${list}" var="member">
						<tr class="trlist" data-id="${member.userNo}">
							...
							<td>${member.name}</td>
							<td class="pwd-mask">
								<!-- 기본적으로 maskedPwd를 표시 --> <span>${member.maskedPwd}</span> <!-- 마우스 오버 시 실제 비밀번호 표시 -->
			...
			<tr>
				<td class="table_bottom button" colspan="9" align="center"><input
					type="button" class="btn" value="회원등록"
					onclick="location.href='${contextPath}/views/main.jsp?url=/views/member/customerInsert.jsp'">
				</td>
			</tr>
		</table>
		...
		<div class="pagination" align="center">
			...
			<c:if test="${pi.currentPage > 1}">
				<a
					href="${contextPath}/customerList.go?currentPage=${pi.currentPage - 1}&status=${param.status}"
			...
			<c:forEach var="pageNum" begin="${pi.startPage}" end="${pi.endPage}">
				<c:choose>
					...
					<c:otherwise>
						<a
							href="${contextPath}/customerList.go?currentPage=${pageNum}&status=${param.status}"
